...
class Dashboard extends Component{
    constructor(props) {
        ...
        this.getTotalTimeSolutions = this.getTotalTimeSolutions.bind(this);
    ...
    getTotalTimeSolutions(){
        ...
        /*Axios.get('http://206.189.220.236:8080/backend-0.0.1-SNAPSHOT/exercises/'+this.props.idUser+'/unpublished')
            .then(response => {
                ...
                _this.setState({ invertedTime: response.data });
                Axios.get('http://206.189.220.236:8080/backend-0.0.1-SNAPSHOT/exercises/'+this.props.idUser+'/unpublished')
                    .then(response => {
                        console.log("soy los ejercicios")
                        console.log(response.data)
                        _this.setState({ solutions: response.data, switcher: true});
            ...
            .then(response => {
                ...
                if( index === this.state.specificChoices.length - 1){
                    ...
                    console.log(JSON.stringify(chartConfigsMultiLine,null,'\t'))
                                ...
                                Axios.get('http://206.189.220.236:8080/backend-0.0.1-SNAPSHOT/statistics/countTime')
                                .then(response => {
                                    console.log("soy los ejercicios")
                                    console.log(response.data)
                                    _this.setState({ invertedTime: response.data });
                                    Axios.get('http://206.189.220.236:8080/backend-0.0.1-SNAPSHOT/solutions/count')
                                        .then(response => {
                                            console.log("soy los ejercicios")
                                            console.log(response.data)
                                            _this.setState({ solutions: response.data, switcher: true});
                                                ...
                                                <Row>
                                                    <Col  xs={6} sm={6} md={3}>
                                                        <Card body inverse color="primary" style={{textAlign:'center'}}>
                                                            <div style={{padding:10}}></div>
                                                            <h1 style={{ color:'white'}}>TIEMPO INVERTIDO TOTAL (horas)</h1>
                                                            <h2 style={{ color:'white'}}>{this.state.invertedTime}<Icon name='time'  size='big'  corner/> </h2>
                                                            <div style={{padding:10}}> </div>
                                            ...
                                            {this.state.singleChartBool &&
                                                 <Row>
                                                    ...
                                                    <Col  xs={6} sm={6} md={3}>
                                                        ...
                                                        <Card body inverse style={{ backgroundColor: '#228B22', borderColor: '#333', textAlign: 'center' }}>
                                                            <div style={{padding:10}}></div>
                                                            <h1 style={{ color:'white'}}>SOLUCIONES TOTALES</h1>
                                                            <h2 style={{ color:'white'}}>{this.state.solutions} </h2>
                                                            <div style={{padding:10}}> </div>
